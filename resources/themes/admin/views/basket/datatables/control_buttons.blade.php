{!! Form::open(["route" => ["admin." . $type . ".destroy", $model->id, 'type' => $basket_type], "id" => "admin_" . $type . "_destroy_form_".$model->id, "method" => "delete", 'class' => 'pull-left']) !!}

    @if ($user->hasAccess((isset($access) ? $access : $type).'.read'))
        <a class="btn btn-info btn-sm btn-flat" href="{!! route('admin.' . $type . '.edit', [$type => $model->id, 'type' => $basket_type]) !!}"
