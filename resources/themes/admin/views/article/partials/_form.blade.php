@include('article.partials._buttons', ['class' => 'buttons-top'])
            ...
            <div class="tab-content">
                @foreach (config('app.locales') as $key => $locale)
                    <div class="tab-pane fade in @if ($key == 0) active @endif" id="tab_{!! $locale !!}">
                        @include('views.article.tabs.locale', ['errors' => $errors, 'model' => $model , 'locale' => $locale])
                    </div>
                @endforeach

                <div class="tab-pane" id="general">
                    @include('article.tabs.general')
                </div>
...
@include('article.partials._buttons')